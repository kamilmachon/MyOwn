...
def simulate(wheel_mass, wheel_radius, density, wheel1_pos, wheel2_pos, wheel_friction, speed1, speed2, pinjoint1_1, pinjoint1_2, pinjoint2_1, pinjoint2_2):
    ...
    while True:
        ...
        if not t%50:
            ...
            for i in range(amount):
                wheels_dist = np.sqrt((track[i].position[0] - track2[i].position[0])**2 + (track[i].position[1] - track2[i].position[1])**2)
        ...
        if time > 12:
            for i in range(amount):
                if int(track[i].position[0]) > 0 and int(track[i].position[0]) < 1700 and int(track[i].position[1]) > 100 and zakazane[i] == 0:
                    probka[i][16] =  int(track[i].position[0])

            break
    for i in range(amount):
        print probka[i][16]
    print "zakazane:", zakazane
...
if __name__ == '__main__':
    simulate(100, 17, 40, 40, 40, 1.5, 15, 15, (20,10), (-20,10), (-20,10), (20,10))
